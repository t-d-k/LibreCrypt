...
<P>
Therefore, it is _recommended_ that you create any new partition based volumes while logged in as an administrator. To do this under Windows Vista, you will need to run DoxBox with elevated permissions; locate the "DoxBox.exe" executable where you installed it, rightclick on this executable and select "Run as administrator" from the context menu.
...
<P>
DoxBox offers users the ability to create "hidden volumes" stored inside other "host" volumes.
...
<P>
Please see the [Plausible Deniability](plausible_deniability.htm) section for further information on the practical uses and considerations of hidden volumes.
...
<P>
_Note: This section only covers the password entry dialog shown when mounting DoxBox volumes. For mounting Linux volumes, please see the section on [Linux volumes](Linux_volumes.htm)._
...
<P>
This option is only available if PKCS#11 support is enabled (see the section on [Security Token/Smartcard Support](pkcs11_support.htm) for more information on how to use this setting.
...
<P>
Selecting the "removable disk" option causes the volume to be mounted as
though it was a removable drive. By mounting volumes in this way, among
other things, files deleted from your volume will not be moved to
a "recycle bin" on your encrypted volume, but will be deleted immediately.

<P>
By default, DoxBox mounts volumes as a fixed disk.
...
</H5>Data from offset includes CDB</H5>
