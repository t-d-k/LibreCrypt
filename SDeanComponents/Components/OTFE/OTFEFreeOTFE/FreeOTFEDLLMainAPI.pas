...
const IOCTL_DISK_GETINFO =
         (((IOCTL_DISK_BASE) * $10000) OR ((FILE_ANY_ACCESS) * $4000) OR (($700) * $4) OR (METHOD_BUFFERED));

const IOCTL_DISK_READ =
         (((IOCTL_DISK_BASE) * $10000) OR ((FILE_READ_ACCESS) * $4000) OR (($702) * $4) OR (METHOD_BUFFERED));

const IOCTL_DISK_WRITE =
         (((IOCTL_DISK_BASE) * $10000) OR ((FILE_WRITE_ACCESS) * $4000) OR (($703) * $4) OR (METHOD_BUFFERED));
...
const IOCTL_FREEOTFE_USER_DEV_HANDLE_SET =
         (((FILE_DEVICE_UNKNOWN) * $10000) OR ((FILE_READ_DATA) * $4000) OR (($815) * $4) OR (METHOD_BUFFERED));

const IOCTL_FREEOTFE_USER_DEV_HANDLE_GET =
         (((FILE_DEVICE_UNKNOWN) * $10000) OR ((FILE_READ_DATA) * $4000) OR (($816) * $4) OR (METHOD_BUFFERED));

const IOCTL_FREEOTFE_SET_FORCE_DISMOUNT =
         (((FILE_DEVICE_UNKNOWN) * $10000) OR ((FILE_READ_DATA) * $4000) OR (($817) * $4) OR (METHOD_BUFFERED));
...
//);
const DLLEXPORT_MAIN_DSK_Init = 'DSK_Init';
type PDSK_Init = function(
  pContext: LPCTSTR;
  lpvBusContext: Pointer
): DWORD; cdecl;

//BOOL DSK_Deinit(
//    DWORD hDevice
//);
const DLLEXPORT_MAIN_DSK_Deinit = 'DSK_Deinit';
type PDSK_Deinit = function(
  hDevice: DWORD
): boolean; cdecl;
...
//    DWORD ShareMode
//);
const DLLEXPORT_MAIN_DSK_Open = 'DSK_Open';
type PDSK_Open = function(
  hDevice: DWORD;
  AccessCode: DWORD;
  ShareMode: DWORD
): DWORD; cdecl;
...
//    DWORD hOpen
//);
const DLLEXPORT_MAIN_DSK_Close = 'DSK_Close';
type PDSK_Close = function(
  hOpen: DWORD
): boolean; cdecl;
...
//    PDWORD pdwActualOut
//);
const DLLEXPORT_MAIN_DSK_IOControl = 'DSK_IOControl';
type PDSK_IOControl = function(
  hOpen: DWORD;
  dwCode: DWORD;
  pBufIn: PBYTE;
  dwLenIn: DWORD;
...
//    WORD Type
//);
const DLLEXPORT_MAIN_DSK_Seek = 'DSK_Seek';
type PDSK_Seek = function(
  hOpen: DWORD;
  Amount: integer;
  xType: WORD
): DWORD; cdecl;
...
//    DWORD Count
//);
const DLLEXPORT_MAIN_DSK_Read = 'DSK_Read';
type PDSK_Read = function(
  hOpen: DWORD;
  pBuffer: PByte;
  Count: DWORD
): DWORD; cdecl;
...
//    DWORD Count
//);
const DLLEXPORT_MAIN_DSK_Write = 'DSK_Write';
type PDSK_Write = function(
  hOpen: DWORD;
  pBuffer: PByte;
  Count: DWORD
): DWORD; cdecl;
...
//    DWORD hDevice
//);
const DLLEXPORT_MAIN_DSK_PowerDown = 'DSK_PowerDown';
type PDSK_PowerDown = function(
  hDevice: DWORD
): DWORD; cdecl;
...
//    DWORD hDevice
//);
const DLLEXPORT_MAIN_DSK_PowerUp = 'DSK_PowerUp';
type PDSK_PowerUp = function(
  hDevice: DWORD
): DWORD; cdecl;
...
//);
const DLLEXPORT_MAIN_DERIVEKEY = 'MainDeriveKey';
type PMainDLLFnDeriveKey = function(
  SizeBufferIn: DWORD;
  DIOCBufferIn: PDIOC_DERIVE_KEY_IN_PC_DLL;
  SizeBufferOut: DWORD;
  DIOCBufferOut: PDIOC_DERIVE_KEY_OUT
): DWORD; cdecl;
...
//);
const DLLEXPORT_MAIN_MACDATA = 'MainMACData';
type PMainDLLFnMACData = function(
  SizeBufferIn: DWORD;
  DIOCBufferIn: PDIOC_GENERATE_MAC_IN_PC_DLL;
  SizeBufferOut: DWORD;
  DIOCBufferOut: PDIOC_GENERATE_MAC_OUT
): DWORD; cdecl;
