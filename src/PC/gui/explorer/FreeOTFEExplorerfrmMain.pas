...
resourcestring
  ...
  RS_COULD_MOUNT_BUT_NOT_PARTITION =
    'Volume could be mounted, but not mounted as a partition image?!';
...
procedure TfrmFreeOTFEExplorerMain.MountFiles(mountAsSystem: TDragDropFileType;
...
begin
  ...
  if (Filesystem <> nil) then begin
    if Filesystem.Mounted then begin
      ExpandRootTreeNode();
      SetupControls();
      SetTitleCaption();
    end;
...
begin
  ...
  if Mounted() then begin
    if (PartitionImage is TOTFEFreeOTFEDLL_PartitionImage) then begin
      volFilename := TOTFEFreeOTFEDLL_PartitionImage(PartitionImage).Filename;
    end else
    if (PartitionImage is TSDPartitionImage_File) then begin
...
procedure TfrmFreeOTFEExplorerMain.actCheckExplorerBarFoldersExecute(Sender: TObject);
begin
  inherited;
...
procedure TfrmFreeOTFEExplorerMain.actCheckToolbarExplorerExecute(Sender: TObject);
begin
  inherited;
...
procedure TfrmFreeOTFEExplorerMain.actCheckToolbarVolumeExecute(Sender: TObject);
begin
  inherited;
...
procedure TfrmFreeOTFEExplorerMain.actPlaintextMountFileExecute(Sender: TObject);
begin
  inherited;
...
function TfrmFreeOTFEExplorerMain.ConfirmOperation(opType: TFExplOperation;
...
procedure TfrmFreeOTFEExplorerMain.actDisconnectNetworkDriveExecute(Sender: TObject);
begin
  inherited;
  //  WebDAVShutdown();
end;
...
begin
  ...
  try
    ...
    if (selItems.Count = 1) then begin
      ...
      if Filesystem.FileExists(selItems[0]) then begin
        // Show properties as single file...
...
procedure TfrmFreeOTFEExplorerMain.SDFilesystemTreeView1Change(Sender: TObject; Node: TTreeNode);
var
  i: Integer;
begin
...
procedure TfrmFreeOTFEExplorerMain.SDFilesystemTreeView1ContextPopup(Sender: TObject;
...
procedure TfrmFreeOTFEExplorerMain.SDFilesystemBothViewStartDrag(Sender: TObject;
...
procedure TfrmFreeOTFEExplorerMain.SDFilesystemTreeView1DragOver(Sender, Source: TObject;
  X, Y: Integer; State: TDragState; var Accept: Boolean);
var
...
procedure TfrmFreeOTFEExplorerMain.SDFilesystemListView1DblClick(Sender: TObject);
begin
  inherited;
...
procedure TfrmFreeOTFEExplorerMain.SDFilesystemListView1DragOver(Sender, Source: TObject;
  X, Y: Integer; State: TDragState; var Accept: Boolean);
begin
  inherited;
...
procedure TfrmFreeOTFEExplorerMain.SDFilesystemBothViewEndDrag(Sender, Target: TObject;
  X, Y: Integer);
var
  droppedOnNode: TTreeNode;
...
procedure TfrmFreeOTFEExplorerMain.SDUDropFilesTreeViewItemsDrop(Sender: TObject;
...
procedure TfrmFreeOTFEExplorerMain.SDUDropFilesListViewItemsDrop(Sender: TObject;
  DropItems: TStringList; DropPoint: TPoint);
var
  droppedOnItem: TListItem;
  importToPath:  WideString;
...
procedure TfrmFreeOTFEExplorerMain.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  inherited;
...
procedure TfrmFreeOTFEExplorerMain.FormCloseQuery(Sender: TObject; var CanClose: Boolean);
begin
  inherited;
  Dismount();
end;
...
// Returns one of: mrYes, mrYesToAll, mrNo, mrCancel
function TfrmFreeOTFEExplorerMain.PromptToReplaceYYANC(srcIsMountedFSNotLocalFS: Boolean;
  srcPathAndFilename: String; destIsMountedFSNotLocalFS: Boolean;
...
function TfrmFreeOTFEExplorerMain.CheckDestinationIsntSource(opType: TFExplOperation;
  srcPathAndFilename: String; destPath: String): Boolean;
var
  ucSrcPath:  String;
  ucDestPath: String;
...
procedure TfrmFreeOTFEExplorerMain.ClipboardSetToSelected(Sender: TObject;
...
procedure TfrmFreeOTFEExplorerMain.ClipboardSetItems(opType: TFExplOperation;
  stlItems: TStringList);
var
  dropType: Word;
begin
...
function TfrmFreeOTFEExplorerMain.ClipboardGetItems(out srcIsMountedFSNotLocalFS: Boolean;
  out opType: TFExplOperation; stlItems: TStringList): Boolean;
...
begin
 ...
 //          should be ABORTED
function TfrmFreeOTFEExplorerMain._PerformOperation(opType: TFExplOperation;
  srcIsMountedFSNotLocalFS: Boolean; srcItem: String; destIsMountedFSNotLocalFS: Boolean;
  destDir: String; destFilename: String;
  // When moving/copying a single file, use this as the destination filename
  // Set to '' to use the source filename's filename
...
function TfrmFreeOTFEExplorerMain._PerformOperation_File(opType: TFExplOperation;
  srcIsMountedFSNotLocalFS: Boolean; srcItem: String; destIsMountedFSNotLocalFS: Boolean;
  destDir: String; destFilename: String;
  // When moving/copying a single file, use this as the destination filename
  // Set to '' to use the source filename's filename
...
begin
  ...
  if (goodToDoOp and not (abortAllRemaining)  // (Included as sanity check)
    ...
    if ((opType = cmCopy) or (opType = cmMove)) then begin
      if destIsMountedFSNotLocalFS then begin
        if Filesystem.FileExists(destItem) then begin
...
function TfrmFreeOTFEExplorerMain._PerformOperation_File_Actual(opType: TFExplOperation;
  srcIsMountedFSNotLocalFS: Boolean; srcItem: String; destIsMountedFSNotLocalFS: Boolean;
  destDir: String; destItem: String; moveDeletionMethod: TMoveDeletionMethod): Boolean;
...
function TfrmFreeOTFEExplorerMain._PerformOperation_Dir(opType: TFExplOperation;
  srcIsMountedFSNotLocalFS: Boolean; srcItem: String; destIsMountedFSNotLocalFS: Boolean;
  destDir: String; destFilename: String;
  // When moving/copying a single file, use this as the destination filename
  // Set to '' to use the source filename's filename
